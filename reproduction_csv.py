...
def load_kNN_model(org_diff_code, tf_diff_code, ref_msg, topK=None):
    ...
    for i, (_) in enumerate(tqdm(run_tqdm)):   
        ...
        trdiff.append(train_msg)
        tsdiff.append(test_msg)
    return blue_scores, trdiff, tsdiff
...
def read_args():
    ...
    parser.add_argument('-topK', type=int, default=100, help='the value of k')
    ...
    parser.add_argument('-csv_name', type=str, default='top100_original.csv', help='the name of csv file')
    return parser
...
if __name__ == '__main__':
    ...
    blue_scores, train_msg, test_msg = load_kNN_model(org_diff_code=org_diff_data, tf_diff_code=tf_diff_data, ref_msg=ref_data,topK=topk)
    data = [train_msg,test_msg]
    export_data = zip_longest(*data, fillvalue = '')
    name_csv = params.csv_name
    with open(name_csv, 'w', encoding="ISO-8859-1", newline='') as file:
        write = csv.writer(file)
        write.writerow(("train_msg", "pred_msg"))
