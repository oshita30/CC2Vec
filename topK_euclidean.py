...
def finding_topK(euclid_dist, topK):
    euclid_dist = list(euclid_dist)
    topK_index = list()
    for i in range(topK):
        min_ = min(euclid_dist)
        index = euclid_dist.index(min_)
        topK_index.append(index)
        del euclid_dist[index]
    return topK_index
...
def load_kNN_model(org_diff_code, tf_diff_code, ref_msg, topK=None):
    ...
    for i, (_) in enumerate(tqdm(run_tqdm)):   
        ...
        euclid_dist = euclidean_distances(X=tf_diff_train, Y=element)

        if topK == None:
            bestK = finding_bestK(diff_trains=org_diff_train, diff_test=org_diff_test[i], topK_index=topK)
        else:
            topK_index = finding_topK(euclid_dist=euclid_dist, topK=topK)
...
if __name__ == '__main__':
    ...
    a_file = open("dict_k_euclidean.pkl", "wb")
